...
impl From<I2cErrorHal> for I2cError<I2cErrorHal> {
    fn from(i2c: I2cErrorHal) -> Self {
        I2cError::I2c(i2c)
    }
}
...
pub struct BME280<'a> {
    ...
    i2c: &'a mut WriteRead<Error = NbError<I2cErrorHal>>,
    /// I²C device address
    address: u8,
...
impl<'a> BME280<'a> {
    ...
    pub fn init(&mut self) -> Result<(), I2cError<I2cErrorHal>> {
        self.verify_chip_id()?;
        self.soft_reset()?;
        self.calibrate()?;
        self.configure()
    ...
    fn verify_chip_id(&mut self) -> Result<(), I2cError<I2cErrorHal>> {
        let chip_id = self.read_register(BME280_CHIP_ID_ADDR)?;
        if chip_id == BME280_CHIP_ID || chip_id == BMP280_CHIP_ID {
            Ok(())
        } else {
            Err(I2cError::UnsupportedChip)
        }
    }

    fn soft_reset(&mut self) -> Result<(), I2cError<I2cErrorHal>> {
        self.write_register(BME280_RESET_ADDR, BME280_SOFT_RESET_CMD)?;
        self.delay.delay_ms(2_u8); // startup time is 2ms
        Ok(())
    }

    fn calibrate(&mut self) -> Result<(), I2cError<I2cErrorHal>> {
    ...
    fn configure(&mut self) -> Result<(), I2cError<I2cErrorHal>> {
        match self.mode()? {
            SensorMode::Sleep => {}
            _ => self.soft_reset()?,
        };
    ...
    fn mode(&mut self) -> Result<SensorMode, I2cError<I2cErrorHal>> {
        let mut data: [u8; 1] = [0];
        block!(self
            .i2c
        ...
        match data[0] & BME280_SENSOR_MODE_MSK {
            ...
            _ => Err(I2cError::InvalidData),
        }
    }

    fn forced(&mut self) -> Result<(), I2cError<I2cErrorHal>> {
        self.set_mode(BME280_FORCED_MODE)
    }

    fn set_mode(&mut self, mode: u8) -> Result<(), I2cError<I2cErrorHal>> {
        match self.mode()? {
            SensorMode::Sleep => {}
            _ => self.soft_reset()?,
        };
    ...
    pub fn measure(
        ...
        match self.calibration.as_mut() {
            ...
            None => Err(I2cError::NoCalibrationData),
        }
    }

    fn read_register(&mut self, register: u8) -> Result<u8, I2cError<I2cErrorHal>> {
        let mut data: [u8; 1] = [0];
    ...
    fn read_pt_calib_data(
        &mut self,
        register: u8,
    ) -> Result<[u8; BME280_P_T_CALIB_DATA_LEN], I2cError<I2cErrorHal>> {
        let mut data: [u8; BME280_P_T_CALIB_DATA_LEN] = [0; BME280_P_T_CALIB_DATA_LEN];
        block!(self.i2c.write_read(self.address, &[register], &mut data))?;
        Ok(data)
    }
    ...
    fn read_h_calib_data(
        &mut self,
        register: u8,
    ) -> Result<[u8; BME280_H_CALIB_DATA_LEN], I2cError<I2cErrorHal>> {
        let mut data: [u8; BME280_H_CALIB_DATA_LEN] = [0; BME280_H_CALIB_DATA_LEN];
        block!(self.i2c.write_read(self.address, &[register], &mut data))?;
        Ok(data)
    }

    fn write_register(&mut self, register: u8, payload: u8) -> Result<(), I2cError<I2cErrorHal>> {
        block!(self
            .i2c
            .write_read(self.address, &[register, payload], &mut []))?;
        Ok(())
    }
}
